...
class DocxProcessor:
    ...
    def normalize_first_paragraph_style(
        ...
        使用纯标准库实现：先从 styles.xml 解析样式名到 styleId 的映射，
        再把 document.xml 中对应的 pStyle 指向目标样式，避免引入 python-docx。
        ...
        try:
            ...
            with zipfile.ZipFile(input_stream, "r") as zin:
                styles_xml = zin.read(styles_path)
                ...
                for style in styles_root.findall(".//w:style", namespaces):
                    ...
                    if style_id and name is not None:
                        style_ids[name.get(f"{{{_WORD_NS}}}val")] = style_id

                source_id = style_ids.get(source_style)
                target_id = style_ids.get(target_style)
                if not source_id or not target_id:
                    log(f"Styles not found: '{source_style}' -> '{target_style}'")
                    return docx_bytes

                root = ET.fromstring(document_xml)
                modified_count = 0
                for pstyle in root.findall(".//w:pStyle", namespaces):
                    if pstyle.get(f"{{{_WORD_NS}}}val") == source_id:
                        pstyle.set(f"{{{_WORD_NS}}}val", target_id)
                        modified_count += 1
            ...
            log(f"Changed {modified_count} paragraph(s) from '{source_style}' to '{target_style}'")
