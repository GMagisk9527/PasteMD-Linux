...
## 0.6.3-linux（2026-09-24）

### 改进

- 转换过程中可主动取消；Pandoc 与自定义过滤器在独立进程组运行，取消时一并终止，剪贴板只在最终提交阶段更新
- 自动粘贴因焦点变化或快捷键未释放失败时，可通过全局热键或托盘菜单手动重试；剪贴板变化不会触发重试循环
- 设置文件损坏时先以唯一文件名备份，再恢复默认配置；备份失败时保留原文件
- 图片未嵌入时显示准确数量及可能相关的来源，不再猜测具体缺失 URL
- 登录自启和应用菜单 `.desktop` 文件改为原子替换，避免写入中断产生残缺文件
- CI 与 Release Actions 升级至 Node.js 24 兼容版本

### 测试

- 新增取消 Pandoc/过滤器进程组、最终剪贴板提交边界、自启原子写入、损坏设置恢复及图片来源诊断测试；全量 133 项通过（1 项跳过）
