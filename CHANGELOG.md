...
## 0.6.2-linux（2026-09-24）

### 改进

- 设置页增加滚动区域，小屏幕也能找到规则与保存按钮；转换时显示读取、解析、生成、写入剪贴板与等待粘贴的阶段提示
- 托盘新增「清理旧缓存 DOCX」：用户确认后只删除默认缓存目录内超过 7 天的 `paste-*.docx`，不碰自定义保存目录

### 修复

- HTML 工作流改为把 HTML 源码按纯文本写入剪贴板；关闭代码高亮使用 Pandoc `--no-highlight`，不再沿用默认着色
- 剪贴板私有标记丢失时比对实际内容，不再因为格式同为文本/HTML 就自动粘贴或复用旧表格；HTML 文本工作流不再误报 `text/plain` 缺失
- 网页表格的纯文本只有 TSV 时改从 HTML 表格提取；同一秒保存的 DOCX 使用唯一文件名避免覆盖
- 应用规则中的带点完整类名必须全名匹配，不再因共有 `com` 等片段命中其他应用；长代码围栏中的短围栏不再使公式预处理改写代码
- 额外保留 DOCX 文件失败时仍继续自动粘贴，并在界面报告保存错误
...
- 106 → 127 项：增加剪贴板安全性、HTML 纯文本、代码围栏、高亮选项、缓存清理、阶段提示与设置页滚动回归测试
