...
3. 按 `Ctrl+Shift+B`。
4. 等待 PasteMD 完成转换并自动粘贴。同一段内容可再按热键重复粘贴；复制新内容后才会重新转换。
...
默认写入 WPS 原生剪贴板，不打开中间文档。转换成功后原剪贴板会被替换；热键再次触发时
若剪贴板仍是这次结果，会直接再粘贴（换到表格/文字窗口则从原文重转）。复制新内容后
剪贴板服务退出，下一次热键按新来源转换。
...
界面提供输入格式选择、公式测试、DOCX 备选输出、快捷键配置、自动粘贴、登录自启和运行记录；转换时会提示当前阶段（读取、转换、写入剪贴板等）。转换过程中可取消 Pandoc/过滤器；取消不会改动剪贴板。进入最终剪贴板写入阶段后将无法取消。默认快捷键是 `Ctrl+Shift+B`。
...
前台是表格但剪贴板里没有 Markdown/网页表时，自动改走文档流（公式/正文仍可粘贴），
不再报「没有 Markdown 表格」。前台是 WPS 文字或其他窗口时仍是文档流程。开关在设置页
